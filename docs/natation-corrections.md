...
- **Matériel** : ne **jamais** coller de matos aléatoire sur le titre de bloc. Matos **dans la ligne d’exo** seulement, format **`avec palmes` / `avec pull-buoy`**. **Interdit dans une même séance** : pull-buoy + palmes (pull = bras seuls ; palmes = battements). Posséder les deux OK. **Quand l’utiliser** : planche sur les jambes ; palmes sur le roulis (jamais plaquettes) ; **tuba = OK s’il est noté dans l’Excel sur l’éducatif** (flèche / chien / croisement / autre) ; le moteur ne **colle pas** de tuba sur un beat/tempo (3T, 5T, 7T, 9T) — le tuba coupe le compte. Pull sur le corps aérobie si pas de palmes ce jour-là. **Engagement** : inventaire non vide → ≥1 item visible (sauf récup / taper / course). « Aucun » = zéro matos.
...
2b. **Matériel** : pas de matos aléatoire sur le titre ; jamais **pull-buoy + palmes dans la même séance** ; inventaire = plusieurs ou aucun ; inventaire déclaré → matos **visible** (`avec …`) hors récup/taper/course. **Ne pas coller tuba sur un beat/tempo (3T/5T/7T/9T)** ; tuba **OK** s’il est noté dans l’Excel sur l’éducatif.
...
29. **Matos × éducatif** : le matériel suit l’éducatif, il n’est pas tiré au sort. Jambes → **planche** (sinon palmes) ; roulis → **palmes** (jamais plaquettes) ; **tuba = source Excel** (noté sur l’éducatif) — flèche / chien / croisement / autre ; le moteur ne **invente pas** tuba sur un beat/tempo (3T, 5T, 7T, 9T). Rattrapé → palmes optionnelles / plaquettes en sportif. **Interdit pull + palmes** dans la même séance (contradictoire : pull coupe les jambes, palmes les utilisent). Matos **sur la ligne d’éducatif**, pas sur la nage appliquée ni le titre. « sans planche » ≠ exige une planche. Cycle ~3/8 jambes.
