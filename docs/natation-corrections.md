...
- **Matériel** : ne **jamais** coller de matos aléatoire sur le titre de bloc. Matos **dans la ligne d’exo** seulement, format **`avec palmes` / `avec pull-buoy`**. **Interdit dans une même séance** : pull-buoy + palmes (pull = bras seuls ; palmes = battements). Posséder les deux OK. **Quand l’utiliser** : planche sur les jambes ; palmes sur le roulis (jamais plaquettes) ; **tuba frontal** sur flèche / grand chien / aisance débutant (tête fixe, facilite la nage) — **jamais** sur 3T/5T/7T (respiration latérale : N coups de bras puis tourner la tête) ; pull sur le corps aérobie si pas de palmes ce jour-là. **Engagement** : inventaire non vide → ≥1 item visible (sauf récup / taper / course, ou inventaire entièrement interdit pour le focus du jour). « Aucun » = zéro matos.
...
|------|----------|------------|--------|
| 2026-08-14 | Tuba ≠ 3T/5T | Arthur : 3T/5T = N coups de bras puis respiration (tête qui tourne) — **interdit** avec tuba frontal (tête fixe). Tuba réservé flèche / grand chien / aisance débutant. `pedagogicalTechEquipment(respiration)=[]` ; forbid tuba/palmes/plaquettes ; pas de glue tuba sur lignes 3T. | ✅ |
...
29. **Matos × éducatif** : le matériel suit l’éducatif, il n’est pas tiré au sort. Jambes → **planche** (sinon palmes) ; roulis → **palmes** (jamais plaquettes) ; **3T/5T/bilatéral → aucun tuba** (la tête doit tourner pour respirer ; le tuba frontal fixe la tête et annule l’exo) ; flèche / grand chien / Découverte → **palmes + tuba** OK (aisance, tête fixe) ; rattrapé → palmes optionnelles / plaquettes en sportif. **Interdit pull + palmes** dans la même séance (contradictoire : pull coupe les jambes, palmes les utilisent). Matos **sur la ligne d’éducatif**, pas sur la nage appliquée ni le titre. « sans planche » ≠ exige une planche. Cycle ~3/8 jambes.
33. **Tuba ≠ 3T** : ne jamais écrire `3T` / `5T` / `7T` **avec tuba frontal**. 3T = 3 coups de bras puis respiration (tête qui tourne). Tuba = respiration continue sans bouger la tête — utile pour débutants (flèche, grand chien, position), pas pour travailler le rythme respiratoire.
