...
- Distances **multiples de la longueur de bassin** (`snap`, `pool` 25 ou 50 m). Totaux annoncés en **XX00 / XX50 uniquement** (jamais XX25 / XX75). Chaque bloc nageable a une **distance explicite** (pas d’échauffement / retour « en minutes » sur une séance en mètres). Somme des blocs = volume affiché. Sous-consignes par 25 m = distance du rep ; références internes cohérentes (`4×100` ne parle jamais de `200 m`). `block()` recalcule depuis les lignes + arrondi ×25. Moteur coach : `profile.pool` passé via `opts.pool` ; pas de séries `Nx25m` en bassin 50 (variantes 50m / adaptation technique). Validation bloquante : `session-coherence.js` via `finalizeCoachSession` / `enforceSessionCoherence`.
...
|------|----------|------------|--------|
| 2026-08-14 | Cohérence distances séances | Validation bloquante + correctifs : chaque bloc a des mètres ; somme = total ; patterns par 25 m = rep ; refs internes (`dernier Xm`) = distance du rep ; total finit en 00/50 (jamais 25/75) ; plus d’échauffement/retour en minutes. Module `session-coherence.js`, branché dans `finalizeCoachSession` + affichage. Fixes banque : `4x150m` + 6ᵉ longueur ; `dernier ${4*P}m`. | ✅ |
...
6. **Distance** : séance qui annonce 2000 m mais détail qui ne tombe pas juste. Source de vérité composeur = **`volumeFromSets`** (séries), recoupée avec `calcDetailsDistance` — pas la cible seule. Totaux **XX00 / XX50** uniquement (jamais 25/75). Chaque bloc (échauffement, éducatifs, nage, série, retour) a une distance en mètres. `4×150` avec 5 longueurs de 25 m = invalide ; `4×100` qui parle du « dernier 200 m » = invalide. **Feedback hebdo** : ne jamais patcher seul `s.distance` sans `details`/`duration` — régénérer ou scale cohérent.
32. **Cohérence séance** : ne jamais afficher un échauffement / retour au calme en minutes sur une séance dont le volume est en mètres. Ne jamais laisser un total finir par 25 ou 75 — ajuster un bloc souple / RAC. Toujours passer `enforceSessionCoherence` / `finalizeCoachSession` avant affichage.
...
31. **4 nages coché** : ne pas transformer la nage préférée en `strokeFocus` (4n + préf. dos ≠ séance 100 % dos). Les quatre nages doivent chacune avoir un bloc nagé nommé. Ne pas supprimer le papillon si `papillonMastered` est false — 4 nages = les 4, papillon fractionné. Interdit de « couvrir » avec un seul intitulé `4 nages` / `plusieurs nages` / `crawl / dos / brasse / papillon`. **Exception IM** : `100/200/400 m 4 nages enchaîné` avec le détail olympique (papillon → dos → brasse → crawl) est explicite.
