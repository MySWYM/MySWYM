...
- **Matériel** : ne **jamais** coller de matos aléatoire sur le titre de bloc. Matos **dans la ligne d’exo** seulement, format **`avec palmes` / `avec pull-buoy`**. **Interdit dans une même séance** : pull-buoy + palmes (pull = bras seuls ; palmes = battements). Posséder les deux OK. **Quand l’utiliser** : planche sur les jambes ; palmes sur le roulis (jamais plaquettes) ; tuba sur alignement / flèche / chien / croisement (pas sur un compte de respiration) ; pull sur le corps aérobie si pas de palmes ce jour-là. **Beat / tempo (3T, 5T, 7T, 9T)** : **jamais avec tuba** — le tuba coupe le compte. **Engagement** : inventaire non vide → ≥1 item visible (sauf récup / taper / course). « Aucun » = zéro matos.
...
| 2026-08-14 | Éducatifs + matos | Cycle technique composeur (~3/8 jambes, chiens rare). Matos pédagogique : planche/jambes, palmes/roulis, tuba/alignement (flèche·chien·croisement) — **pas** sur 3T/5T/7T/9T. Drills banque qui portent déjà l’outil ; matos seulement sur la ligne d’éducatif. Jamais pull+palmes. « sans planche » ne requiert plus de planche. Pas de regen. | ✅ |
...
| 2026-08-15 | Beat/tempo ≠ tuba | Arthur : dès qu’on demande un beat/tempo (3T, 5T, 7T, 9T), **pas de tuba** sur la même ligne — le tuba coupe le compte de respiration. Tuba reste OK sur flèche / chien / croisement / alignement. Glue + sanitize + QG. Pas de regen. | ✅ |
...
2b. **Matériel** : pas de matos aléatoire sur le titre ; jamais **pull-buoy + palmes dans la même séance** ; inventaire = plusieurs ou aucun ; inventaire déclaré → matos **visible** (`avec …`) hors récup/taper/course. **Jamais beat/tempo (3T/5T/7T/9T) + tuba** sur la même ligne.
...
29. **Matos × éducatif** : le matériel suit l’éducatif, il n’est pas tiré au sort. Jambes → **planche** (sinon palmes) ; roulis → **palmes** (jamais plaquettes) ; alignement / flèche / chien / croisement → **tuba** ; **jamais tuba sur un beat/tempo** (3T, 5T, 7T, 9T — le tuba coupe le compte). Rattrapé → palmes optionnelles / plaquettes en sportif. **Interdit pull + palmes** dans la même séance (contradictoire : pull coupe les jambes, palmes les utilisent). Matos **sur la ligne d’éducatif**, pas sur la nage appliquée ni le titre. « sans planche » ≠ exige une planche. Cycle ~3/8 jambes.
