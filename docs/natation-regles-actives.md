...
9. **Matériel** — Le matériel déclaré indique ce que le nageur peut utiliser, jamais ce qu’il doit utiliser. Une séance peut être sans matériel même si l’inventaire est rempli. Lorsqu’un matériel est utilisé, il doit être affiché explicitement sur la ligne d’exercice concernée. Jamais pull-buoy + palmes **dans le même exercice** (même ligne) ; les deux le même jour sur des lignes distinctes est OK. Jamais élastique chevilles + palmes, ni élastique + planche, **même ligne**. Pull + élastique **autorisé** (plus facile au début). Finger paddles (`plaquettes_doigts`) = technique / appui ; plaquettes = force / traction ; un seul des deux peut couvrir l’autre rôle. Matériel lié à l’éducatif, jamais tiré au sort.
