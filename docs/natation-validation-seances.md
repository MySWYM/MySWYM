...
- **Calendrier event Sheet (XS/Sprint…)** : rôle de semaine depuis `eventDate` (S0 = semaine du jour J). **S0 + S-1** = deload (S0 : max 2 séances) ; **S-2 → S-5** = construction (pas de test/allégée cycle) ; **S-6+** = cycle **6 travail → allégée → test** ancré sur J (**S-7 allégée, S-6 test** ; ex. S-15 allégée → S-14 test → S-13…S-8 travail). Début de plan : **2 sem. travail** avant test/allégée cycle (S0/S-1 intacts). Sans date = même cycle + garde. Bandeau séance + planning accueil (`sheetWeekRole` / `EventWeekPlanCard`).
