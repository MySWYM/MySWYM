...
- **Calendrier event Sheet (XS/Sprint…)** : rôle de semaine depuis `eventDate` (S0 = semaine du jour J). **S0 + S-1** = deload (S0 : max 2 séances) ; **S-2 → S-6** = construction (pas de test) ; **S-7+** = cycle **6 travail → allégée → test** ancré sur J (**S-8 allégée, S-7 test** ; ex. S-16 allégée → S-15 test → S-14…S-9 travail). Début de plan : **4 sem. travail** avant test/allégée cycle (S0/S-1 intacts). Sans date = même cycle + garde. Bandeau séance + planning accueil (`sheetWeekRole` / `EventWeekPlanCard`).
