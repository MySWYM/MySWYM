...
- **Pace Sheet** : sur les lignes, tokens `{D:moyen}` / `{D:vite}` / `{D:course}` / `{D:souple}` et `{@:…}` (même intents). L’app remplace par un départ `D…` et une plage `@mm:ss-mm:ss` calculés depuis le **T100**, si nageur **intermédiaire ou avancé + Premium + T100**. **Débutant = jamais de pace** (tous objectifs) : `{D:…}` → `repos 30 s`, `{@:…}` retiré. Ne pas mettre ces tokens sur les onglets débutant (`01`, `04`) sauf si tu acceptes le fallback repos.
