...
- **Calendrier event Sheet (XS/Sprint…)** : rôle de semaine depuis `eventDate` (S0 = semaine du jour J). **S0 + S-1** = deload (S0 : max 2 séances) ; **S-2 → S-6** = construction (pas de test) ; **S-7+** = cycle 7 construction → 1 test (chronos 50/100/400) → 1 deload. Sans date = cycle seul. Bandeau dans la vue séance (`sheetWeekRole`).
