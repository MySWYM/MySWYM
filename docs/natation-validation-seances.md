...
- **Calendrier event Sheet (XS/Sprint…)** : rôle de semaine depuis `eventDate` (S0 = semaine du jour J). **S0 + S-1** = deload (S0 : max 2 séances) ; **S-2 → S-6** = construction (pas de test) ; **S-7+** = cycle **6 travail → test → allégée** ancré sur J (**S-7 = test** ; ex. S-15 test → S-14 allégée → S-13…S-8 travail). Début de plan : **4 sem. travail** avant test/allégée cycle (S0/S-1 intacts). Sans date = même cycle + garde. Bandeau séance + planning accueil (`sheetWeekRole` / `EventWeekPlanCard`).
