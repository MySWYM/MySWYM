...
const LIGHT = {
  ...
  loginBg: "#f2f3f6",
  loginBorder: "#c3c6d2",
  overlay: "rgba(25,28,30,0.28)",
};

const DARK = {
  ink: "#eef3fb",
  panel: "#102038",
  secondary: "rgba(238,243,251,0.65)",
  border: "rgba(142,179,255,0.16)",
  navBg: "rgba(11, 21, 36, 0.92)",
  accent: "#8eb3ff",
  accentText: "#0b1524",
  loginBg: "#17304a",
  loginBorder: "rgba(142,179,255,0.28)",
  overlay: "rgba(0,0,0,0.55)",
};

export default function PublicNav({ onDark = false }) {
  ...
  return (
    <>
      <nav style={{
        ...
        boxShadow: scrolled ? (onDark ? "0 1px 20px rgba(0,0,0,0.35)" : "0 1px 20px rgba(142,179,255,0.12)") : "none",
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0, minWidth: 0 }}>
            {!isMobile && <LanguageSwitcher variant="nav" onDark={onDark} />}
            {!isMobile && (
      ...
      {isMobile && (
        <div style={{ position: "fixed", top: 64, left: 0, right: 0, bottom: 0, zIndex: 199, pointerEvents: menuOpen ? "all" : "none" }}>
          <div onClick={() => setMenuOpen(false)} style={{ position: "absolute", inset: 0, background: C.overlay, opacity: menuOpen ? 1 : 0, transition: "opacity 0.25s" }} />
          <div style={{
            position: "absolute", top: 0, left: 0, right: 0, background: C.panel, borderBottom: `1px solid ${C.border}`,
            boxShadow: menuOpen ? "0 8px 32px rgba(0,0,0,0.28)" : "none", padding: "8px 0 24px",
            ...
            <div style={{ padding: "20px 24px 0", display: "flex", flexDirection: "column", gap: 10, alignItems: "center" }}>
              <LanguageSwitcher variant="nav" onDark={onDark} />
              ...
              <a href="/connexion" onClick={() => setMenuOpen(false)} style={{ display: "block", width: "100%", textAlign: "center", padding: "13px", borderRadius: 16, border: `1.5px solid ${C.loginBorder}`, color: C.ink, fontSize: 15, fontWeight: 600, textDecoration: "none", background: C.loginBg, fontFamily: "'Lexend', sans-serif", boxSizing: "border-box" }}>
                {t("nav.login")}
              </a>
            </div>
          </div>
